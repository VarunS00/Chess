...
class Rook(Piece):
    def __init__(self, row, col, color, image=None):
        ...
        #Sets the name attribute to "rook"
        self.name = "rook"
    #Returns legal moves for the rook
    def get_moves(self, board):
        #Creates an empty list to store moves
        moves = []
        #Defines the possible directions the rook can move (up, down, left, right)
        directions = [
            (-1, 0),
            (1, 0),
            (0, -1),
        ...
        #Iterates through each direction to calculate possible moves
        for dr, dc in directions:
            #Initializes the row and column to the current position of the rook
            r = self.row
            c = self.col
